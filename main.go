...
func main() {
	log.Println("Initializing Proofolio with Go & PostgreSQL...")

	// 1. Initialize DB, Run Migrations & Seeding
	_, err := db.InitDB()
	if err != nil {
	...
	// 2. Static Asset Server (CSS, JS, Fonts)
	...
	log.Printf("Proofolio server listening on http://localhost:%s", port)
